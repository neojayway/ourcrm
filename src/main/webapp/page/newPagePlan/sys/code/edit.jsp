...
<body>
	<form name="ActionForm" method="post" action="${pageContext.request.contextPath}/codeRule/updateCodeRule.do">
		<div class="mtitle">
					...
					<div id="menu1">
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							...
							<tr>
								...
								<td>
									<input type="hidden" name="id" value="${requestScope.codeRule.id}">
									<input name="module" type="text" class="disabled"
										id="module" style="width: 90%" readonly
										value="${requestScope.codeRule.module }">
								</td>
								<td class="red">数据库表名：</td>
								<td>
									<input name="tabname" type="text" class="disabled"
										id="tabname" style="width: 90%" readonly
										value="${requestScope.codeRule.tabname }">
								</td>
							</tr>
							<tr>
								...
								<td>
									<input name="areaprefix" type="text" class="input"
										...
										value="${requestScope.codeRule.areaprefix }">
								</td>
								<td class="red">流水位：</td>
								<td>
									<select name='glidebit' id='glidebit'
										...
										<c:if test="${applicationScope.glidebitList!=null}">
											<c:forEach var="dictionaryType" varStatus="s" items="${applicationScope.glidebitList}">
												<c:choose>
													<c:when test="${requestScope.codeRule.glidebit == dictionaryType.value}">
								...
								<td colspan="3">
									<c:if test="${applicationScope.areatimeList!=null}">
										<c:forEach var="dictionaryType" varStatus="s" items="${applicationScope.areatimeList}">
											<c:choose>
												<c:when test="${requestScope.codeRule.areatime == dictionaryType.value}">
													<input type='radio' name='areatime' id='areatime' 
														value='${dictionaryType.value}' class='radio' checked="checked">
