...
<script type="text/javascript">
	...
	function shutdown(){
		window.document.location = '${pageContext.request.contextPath}/user/showUser.do'
	}
...
<body>
	<form name="ActionForm" method="post" action="${pageContext.request.contextPath }/user/saveUserRole.do"
		onSubmit="return check();">
		<input type="hidden" name="uid" value="${requestScope.uid }"> 
		<input type="hidden" name="ridStr" value="${requestScope.ridStr }">
		<div class="mtitle">
							...
							<tr>
								<td>
									<select name="lselect" size="1" multiple id="lslt"
										...
										<c:forEach items="${requestScope.roleOrigin }" var="role">
								...
								<td>
									<select name="rselect" size="1" multiple id="rslt"
										...
										<c:forEach items="${requestScope.roleAllList }" var="role">
											<option value="${role.roleid}" onDblClick="">${role.rolename}</option>
										</c:forEach>
